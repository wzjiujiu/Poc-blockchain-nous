...
async function terminateContrattoOnchain(contract, nodeId, rawPort) {
  try {
    ...
    console.log("🛑 Terminate contratto:", contractAgreementId);

    // ======================= RECUPERO TRANSFER PROCESSES =======================
    const transferUrl = `http://localhost:11000/api/management/v3/transferprocesses/request`;
    const querySpec = {
    ...
    const transferResp = await axios.post(transferUrl, querySpec, {
      headers: {
    ...
    const matchingTransfers = transfers.filter(
      t => t.contractId === contractAgreementId && t.state === "STARTED"
    );

    const transferIds = matchingTransfers.map(t => t['@id']);
    console.log("📦 Transfer IDs da terminare:", transferIds);
    ...
    // In un consumer Kafka gestisci l'errore senza res
